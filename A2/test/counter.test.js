...
counter = new Counter(editor, options);
...
it("count number of letters", () => {
    ...
    expect(counter.calculate()).toEqual(26)
  });


  it("count unit of words test ", () => {
   counter.options.unit='word',
    editor.updateContents([{
        
        insert:'two words three'
    }])
    expect(counter.options.unit=='word').toBeTruthy();
    expect(counter.calculate()).toEqual(3)
  });
  ...
  it("update with inserting the words", () => {
    editor.updateContents([{
        insert:'abcdefghijklmnopqrstuwxyz'
    }])
 
    expect(counter.update()).toMatch('s')
  });

  it("update without inserting any word", () => {
    editor.updateContents([{
        insert:''
    }])
    expect(counter.update()).toMatch('')
  });
