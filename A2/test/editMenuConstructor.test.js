...
it("should create a correct constructor", () => {
    var options = {
        debug: 'info',
        modules: {
    ...
    editsMenu = new EditsMenu(editor, options);
     expect(editsMenu.quill).toBe(editor);
     expect(editsMenu.options).toBe(options);
     expect(editsMenu.container).toBe(document.querySelector(options.container));
     expect(editsMenu.editorContents).toStrictEqual(editor.getContents());
     expect(editsMenu.groups).toStrictEqual([]);
     expect(editsMenu.edits).toStrictEqual([]);
